...
export default function DocesPage() {
    ...
    const listaFiltrada = products.filter((p) => p.category === "bebidas").filter((p) => p.name.toLowerCase().includes(busca.toLowerCase())).filter((p) => (tag ? p.tagIds.includes(tag) : true))
        .sort((a, b) => {
        ...
        <div className="flex flex-col min-h-screen bg-clean-cream">
            ...
            <section className="relative h-[30vh] md:h-[43vh] md:min-h-[350px] w-full flex items-center justify-center overflow-hidden">
                <div className="absolute inset-0 z-0">
                    <Image
                        src="/assets/bannerbebidas.jpg"
                        alt="Vitrine de bebidas"
                        fill
                ...
                <div className="relative z-10 text-center text-white space-y-4 px-4 animate-in fade-in slide-in-from-bottom-6 duration-700">
                    ...
                    <h1 className="text-5xl md:text-7xl font-serif leading-tight">
                        Nossas <span className="italic font-light">Bebidas</span>
                    </h1>
                    <p className="text-white/80 text-lg max-w-lg mx-auto font-light">
                        Para aquecer a alma ou refrescar o dia.
                    </p>
                </div>
            </section>
            ...
            {/*buscador*/}
            <section className="md:sticky top-0 z-30 bg-soft-white/70 backdrop-blur-md border-b border-mocha/10 shadow-sm transition-all">
                <Buscador
                    busca={busca}
                    setBusca={setBusca}
                    tag={tag}
