...
export function Buscador({ busca, setBusca, tag, setTag, ordenador, setOrdenador }: props) {
    ...
    return (
        <section className="mb-2 md:mb-8 space-y-4 w-full">

            <div className="bg-soft-white rounded-lg flex items-center shadow-sm border border-mocha/10 p-2 mx-4 md:mx-21 lg:mx-40 my-4">
                <input
            ...
            <section className="md:flex justify-between mx-4 lg:mx-40">
                <div className="flex flex-wrap gap-2 transition-all duration-300 justify-center mb-2 md:mb-0">
                    {tagsParaExibir.map((tagb) => (
                        <button
                            key={tagb.id}
                            onClick={() => handleTagClick(tagb.id)}
                            className={`${getTagColor(tagb.id)} text-white px-2.5 md:px-5 py-2 rounded-full text-sm font-semibold transition-all duration-300 ${tag === tagb.id ? "ring-2 ring-offset-2 ring-mocha/20 scale-105" : ""}`}>
